...
fun FirstUI(modifier: Modifier = Modifier) {
    ...
    var filteredList by remember { mutableStateOf(itemList.toList()) }
    ...
    Column(
        ...
        SearchInputBar(
            ...
            onSearch = {
                filteredList = if (it.isNotBlank()) {
                    itemList.filter { item -> item.contains(it, ignoreCase = true) }
                } else {
                    itemList.toList()
                }
            }
        ...
        CardsList(displayedItems = filteredList)
    }
}
